...
- Operations lacking a strict executable contract remain in the administrative registry and preflight status; they are not deleted and are not registered in active action enums. Exactly eighteen reads are executable today: the eleven existing Outlook/Calendar, SharePoint/OneDrive and Teams reads, plus `todo.list_task_lists`, `todo.search`, `todo.read`, `planner.list_plans`, `planner.list_buckets`, `planner.list_tasks` and `planner.read`. Every write stays non-executable until the host approval fix under "Host dependency" lands.
